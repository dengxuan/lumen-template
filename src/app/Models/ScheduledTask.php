...
use App\Jobs\PingOhDearJob;
use App\Support\Concerns\UsesScheduleMonitoringModels;
use App\Support\ScheduledTasks\ScheduledTaskFactory;
...
class ScheduledTask extends Model
{
    ...
    public function markAsRegisteredOnOhDear(): self
    {
        if (is_null($this->registered_on_oh_dear_at)) {
            $this->update(['registered_on_oh_dear_at' => Date::now()]);
        }
    ...
    public function markAsFinished(ScheduledTaskFinished $event): self
    {
        ...
        $this->update(['last_finished_at' => Date::now()]);
    ...
    public function markAsFailed($event): self
    {
        ...
        $this->update(['last_failed_at' => Date::now()]);
    ...
    public function markAsSkipped(ScheduledTaskSkipped $event): self
    {
        ...
        $this->update(['last_skipped_at' => Date::now()]);

        return $this;
    }

    public function pingOhDear(ScheduledTaskLogItem $logItem): self
    {
    ...
    public function createLogItem(string $type): ScheduledTaskLogItem
    {
    ...
    {
        if (!(is_null($event->task->output) ?? false)) {
            return null;
        }
